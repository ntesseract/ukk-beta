<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
                <div class="mb-4 md:mb-0">
                    <h1 class="text-3xl font-bold text-gray-900">List Produk</h1>
                </div>
            
                <div class="hidden md:flex items-center gap-4">
                    <form class="flex-1" action="{{ route('products.index') }}" method="GET">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300">
                    <div class="h-48 relative">
                        <img 
                            src="{{ $product->foto ? url('storage/' . $product->foto) : asset('default-product.jpg') }}" 
                            class="w-full h-full object-cover"
                            alt="{{ $product->nama }}"
                            loading="lazy"
                        >
                        <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-4">
                            <h3 class="text-xl font-bold text-white">{{ $product->nama }}</h3>
                            <p class="text-sm text-blue-200">NIS: {{ $product->nis }}</p>
                        </div>
                    </div>
                    ...
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content rounded-xl overflow-hidden border-0">
                            <div class="modal-header bg-gradient-to-r from-blue-600 to-blue-500 p-6">
                                <h5 class="text-xl font-bold text-white">Detail Produk</h5>
                            ...
                            <div class="modal-body p-6">
                                <div class="flex flex-col md:flex-row gap-8">
                                    <div class="md:w-1/3">
                                        ...
                                        <div class="mt-4 p-4 bg-white rounded-lg shadow text-center">
                                            <h6 class="text-sm font-semibold text-gray-600 mb-2">Barcode</h6>
                                            <center>{!! $product->generateBarcode() !!}</center>
                                            <p class="mt-2 text-sm font-mono text-center">{{ $product->nis }}</p>
                                        </div>
                                    </div>
                                    <div class="md:w-2/3 space-y-4">
                                        <div>
                                            <label class="text-sm font-medium text-gray-500">Nama Produk</label>
                                            <p class="text-xl font-semibold text-gray-800">{{ $product->nama }}</p>
                                        </div>
                                        
                                        <div class="grid grid-cols-2 gap-4">
                                            <div>
                                                <label class="text-sm font-medium text-gray-500">NIS</label>
                                                <p class="text-gray-800">{{ $product->nis }}</p>
                                            </div>
                                            <div>
                                                <label class="text-sm font-medium text-gray-500">Harga</label>
                                                <p class="text-gray-800">Rp {{ number_format($product->harga) }}</p>
                                            </div>
                                            <div>
                                                <label class="text-sm font-medium text-gray-500">Tanggal Masuk</label>
                                                <p class="text-gray-800">
                                                    {{ date('d F Y', strtotime($product->tanggal)) }}
                                                </p>
                                            </div>
                                            <div>
                                                <label class="text-sm font-medium text-gray-500">Stok Tersedia</label>
                                                <p class="text-gray-800">
                                                    {{ number_format($product->jumlah) }}
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="pt-4">
                                            <button 
                                                class="w-full bg-blue-500 text-white py-2 rounded-md hover:bg-blue-600 transition-colors font-medium"
                                                type="button"
                                                data-bs-toggle="modal" 
                ...
                <div class="modal fade" id="editProductModal{{ $product->id }}" tabindex="-1">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content rounded-xl">
                            <div class="modal-header bg-gradient-to-r from-blue-600 to-blue-500 p-6">
                                <h5 class="text-xl font-bold text-white">Edit Produk</h5>
                                <button type="button" class="text-white opacity-75 hover:opacity-100" data-bs-dismiss="modal">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                    </svg>
                                                ...
                                                <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity rounded-xl">
                                                    <label class="cursor-pointer text-white">
                                                        <input 
                                                            ...
                                                            class="hidden"
                                                            accept="image/*"
                                                            onchange="document.getElementById('previewFoto{{ $product->id }}').src = window.URL.createObjectURL(this.files[0])">
                                            ...
                                            <div class="grid grid-cols-2 gap-4">
                                                <div>
                                                    ...
                                                    <input 
                                                        ...
                                                        disabled>
                                                    <input type="hidden" name="nis" value="{{ $product->nis }}">
                                                </div>
                                                <div>
                        ...
                        <div class="modal-content rounded-xl">
                            ...
                            <div class="modal-body p-6">
                                <div class="text-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900 mb-2">Hapus Data Produk?</h3>
                                    <p class="text-gray-500">Produk {{ $product->nama }} ({{ $product->nis }}) akan dihapus permanen.</p>
                                </div>
                            </div>
                            <div class="modal-footer p-6 border-t border-gray-200">
    ...
    <div class="modal fade" id="addProductModal" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content rounded-xl">
                <div class="modal-header bg-gradient-to-r from-blue-600 to-blue-500 p-6">
                    <h5 class="text-xl font-bold text-white">Tambah Produk Baru</h5>
                    <button type="button" class="text-white opacity-75 hover:opacity-100" data-bs-dismiss="modal">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                ...
                <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-footer p-6 border-t border-gray-200">
                        <button type="button" class="px-6 py-2.5 text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200" data-bs-dismiss="modal">Batal</button>
                        <button type="submit" class="px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 ml-2">Simpan Produk</button>
                    </div>
                </form>
            </div>
        </div>
